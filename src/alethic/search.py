...
class SearchConfig:
    ...
    Mirrors the spec preset table (§Presets, lines 295-302). Kept separate
    from ``AgentConfig`` for the duration of PR #10 — wiring into the main
    config is the next stacked commit. Callers pass ``SearchConfig`` to
    ``solve()`` directly; ``AgentConfig`` continues to drive the per-call
    knobs the underlying GVR machinery already understands (model,
    confidence_threshold, temperature, etc.).
