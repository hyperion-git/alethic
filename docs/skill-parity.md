...
| Error-category keyword classifier + `error_category` in VERIFY events | `error_taxonomy.classify_errors()` | orchestrator.md Classifier 1 |
...
- **Disproof escalation** (v3.7.1) — generator addendum on reset iterations; portable
  in principle via the skill's stall-reset step, currently library-only.
- **Hierarchical inconsistency classifier** (v3.7.2) — skills keep the flat keyword
  classifier; only `error_category` values must remain compatible.
...
Last full audit: 2026-06-10 (found and fixed candidate-selection drift; all prompts at parity).
