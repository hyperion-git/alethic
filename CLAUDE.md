...
```bash
DEST=~/.claude/plugins/cache/local/alethic/3.3.1
mkdir -p "$DEST"
...
| Module | Purpose |
|--------|---------|
| `agent.py` | `MathAgent` orchestrator — runs the Generate N → Verify all → Select best → Revise loop with best-of-N sampling (parallel via `ThreadPoolExecutor`), variant-B model diversity (odd-indexed candidates use alternate config, variant-B client created from stored `_api_key`), FIXABLE verdict shortcut (re-verifies corrected solution, accepts if passing), false-premise detection, candidate ranking, failed approach tracking via `RunState`, structured event logging via `EventLog` (partial candidate failures logged as ERROR events), switchable tool guidance via `_build_system_prompt()`/`_get_tool_guidance_map()`, stall detection with strategy reset via `_check_stall()`/`_build_reset_context()`, error taxonomy wired into revision loop (calls `classify_errors()` → injects `critique_addendum` to reviser), adversarial verifier self-correction via `_adversarial_addendum()` (injects 5-round self-critique protocol when `config.adversarial_self_correction` is True), token ledger integration (records usage from every API response), checkpoint-resume via `resume_from` parameter on `solve()`, `ContextExhaustedError`/`TruncatedResponseError` handling with automatic checkpoint, session directory auto-creation (`.alethic/` or `/tmp/alethic-*/`), and `_domain()` method for subclass identification |
