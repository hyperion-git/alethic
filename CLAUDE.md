...
alethic eval run data/benchmarks/math-sample.json --search tree  # flat-vs-tree gate comparison
...
```bash
DEST=~/.claude/plugins/cache/local/alethic/3.8.0
mkdir -p "$DEST"
...
| Preset | Iters | Revs | Threshold | Best-of | Thinking | Think budget | Max tokens | Stall reset | Window | Epsilon | N boost | Variant B | Ctx thresh | Adversarial verifier |
|--------|-------|------|-----------|---------|----------|-------------|------------|-------------|--------|---------|---------|-----------|------------|---------------------|
| `quick` | 2 | 1 | 0.85 | 1 | off | — | 16,384 | off | — | — | 0 | — | 0.85 | off |
| `default` | 5 | 3 | 0.90 | 2 | off | — | 16,384 | on | 2 | 0.03 | 1 | — | 0.80 | off |
| `thorough` | 8 | 5 | 0.95 | 3 | on | 15,000 | 32,768 | on | 3 | 0.02 | 1 | Sonnet | 0.80 | on |
| `extreme` | 12 | 5 | 0.97 | 5 | on | 40,000 | 65,536 | on | 3 | 0.02 | 2 | Sonnet | 0.75 | on |
...
| Module | Purpose |
|--------|---------|
| `agent.py` | `MathAgent` orchestrator — runs the Generate N → Verify all → Select best → Revise loop with best-of-N sampling (parallel via `ThreadPoolExecutor`), variant-B model diversity (odd-indexed candidates use alternate config, variant-B client created from stored `_api_key`), FIXABLE verdict shortcut (re-verifies corrected solution, accepts if passing), false-premise detection, failed approach tracking via `RunState`, structured event logging via `EventLog` (partial candidate failures logged as ERROR events, VERIFY events include `error_category`), switchable tool guidance via `_build_system_prompt()`/`_get_tool_guidance_map()`, error taxonomy wired into revision loop (calls `classify_errors()` → injects `critique_addendum` to reviser), adversarial verifier self-correction via `_adversarial_addendum()` (injects 5-round self-critique protocol when `config.adversarial_self_correction` is True), token ledger integration (records usage from every API response), checkpoint-resume via `resume_from` parameter on `solve()`, `ContextExhaustedError`/`TruncatedResponseError` handling with automatic checkpoint, session directory auto-creation (`.alethic/` or `/tmp/alethic-*/`), `_domain()` method for subclass identification. Routing functions (stall detection, N-computation, atom context, reset context, rank_candidates) moved to `oracle_router.py` in v3.7; agent delegates to `self.router`; `search_mode="tree"` dispatches to `search.solve()` via `_solve_tree()` (tree-mode sessions, checkpoint/resume) |
| `oracle_router.py` | `RoutingDecision` frozen dataclass + `OracleRouter` class — pure-function routing layer consolidating `check_stall()`, `build_reset_context()`, `build_verifier_extra_system()`, `build_atom_context()`, `rank_candidates()`, `revision_budget()`, and N-computation from `agent.py`. Activates previously dead code: `_ORACLE_ROUTING` table wired into `route()` for oracle type selection, `rank_candidates()` wired into `_verify_candidates()`. `route(state, evidence)` returns `RoutingDecision` for pre-iteration decisions; individual methods available for post-verification decisions. Module-level helpers: `combine()`, `build_atom_focus_directive()` |
...
| `physics_agent.py` | `PhysicsAgent` — thin subclass of `MathAgent` that injects physics-specific prompt templates and overrides `_get_tool_guidance_map()` to return `PHYSICS_TOOL_GUIDANCE`, `_reset_addendum()` to return `PHYSICS_STRATEGY_RESET_ADDENDUM`, and `_adversarial_addendum()` to return `PHYSICS_ADVERSARIAL_VERIFIER_ADDENDUM` |
| `subagents.py` | `generate()`, `verify()`, `revise()` — each wraps a Claude API call with role-specific prompts; accepts optional prompt kwargs for domain specialization; supports extended thinking. `verify()` accepts `extra_system: str | None` for injecting adversarial self-correction protocol. `revise()` accepts `critique_addendum: str | None` for injecting error-taxonomy-driven strategy. `_safe_format()` helper replaces `str.format()` with `str.replace()` to handle curly braces in math problems. `_parse_verification()` regex terminates CORRECTED SOLUTION blocks only on `END CORRECTED SOLUTION` (not arbitrary ALL-CAPS labels) |
| `models.py` | Dataclasses: `AgentConfig` (with `PRESETS`, `from_preset()`, `build_variant_b_config()`, `best_of_n`, `tool_guidance: frozenset[str]` accepting `{"sympy", "numpy", "scipy", "matplotlib"}`, `stall_window`, `stall_epsilon`, `stall_reset`, `reset_n_boost`, `variant_b: dict | None`, `context_threshold: float`, `adversarial_self_correction: bool`, `search_mode: "flat"|"tree"`, `search: SearchConfig | None` fields), `SearchConfig` (v3.8 tree-search parameters, moved from search.py), `VerifierConfig` (with `PRESETS`, `from_preset()`, `num_verifiers`, `domain`, `tool_guidance` fields), `ConsensusResult` (verdict, confidence, confidence_range, critique, issues, individual_results), `ConsensusIssue` (text, severity, flagged_by), `Solution`, `VerificationResult` (with `Issue`, `SectionConfidence`, `corrected_solution: str | None`, severity-aware `is_acceptable()`, `has_correction` property), `Revision`, `AgentResult` (with `AgentEvent` list, `failed_approaches`, `token_ledger`, `session_dir`, `checkpoint_path`), `Verdict` enum (CORRECT, MINOR_ISSUES, FIXABLE, MAJOR_FLAW, UNSOLVED), `IssueSeverity` enum, `EventType` enum (incl. `STALL_RESET`), `TokenLedger` (cumulative token tracking with `record()`, `total_input`/`total_output`, `utilization()`), `MODEL_CONTEXT_LIMITS` dict |
| `prompts.py` | Math system/user prompt templates for all three subagents + balanced prompting addendum + `STRATEGY_RESET_ADDENDUM` + `ADVERSARIAL_VERIFIER_ADDENDUM` (5-round self-critique protocol) + `TOOL_GUIDANCE` map (SymPy/NumPy generator/verifier guidance strings); generator includes `verify_step_N()` embedding instructions; verifier includes backward verification pass, FIXABLE verdict, and citation-checking instructions |
| `physics_prompts.py` | Physics-specific prompt templates: derivation strategies, physics error checklist, dimensional/limiting-case balanced addendum + `PHYSICS_STRATEGY_RESET_ADDENDUM` + `PHYSICS_ADVERSARIAL_VERIFIER_ADDENDUM` + `PHYSICS_TOOL_GUIDANCE` map (physics-specific SymPy/NumPy guidance); generator includes `verify_step_N()` embedding; verifier includes backward verification pass, FIXABLE verdict, and citation-checking instructions |
| `error_taxonomy.py` | `classify_errors(critique: str) -> str` — keyword heuristic classifying verifier critique into one of six categories: `algebra` (sign/arithmetic errors), `logic` (invalid inference), `citation` (vague appeals to authority), `interpretation` (specification gaming), `units` (dimensional mismatch), `missing_case` (incomplete case analysis). Returns a targeted revision strategy addendum string injected by `agent.py` into the reviser's user prompt |
| `autopsy.py` | `generate_autopsy(result, *, api_key, model) -> str` — LLM-synthesized failure report for UNSOLVED sessions. `_classify_failure_pattern(result)` deterministically classifies as `persistent_flaw` / `oscillation` / `regression` / `stall` using VERIFY event history (filters to winning candidate per iteration in best-of-N mode via `"candidate"` key in event data). Uses `_extract_text()` and `_create_with_retry()` from `subagents.py`. Written to `worklog/autopsy.md` by `agent.py` on UNSOLVED exit (try/except guarded). Exported from `__init__.py` |
| `tools.py` | `execute_python()` sandbox, `PYTHON_TOOL` schema (highlights SymPy as `sp` and NumPy as `np`), `process_tool_calls()` for tool-use loop. `_BLOCKED_SUBMODULES` blocklist in `_restricted_import` prevents escape via dotted imports (e.g., `numpy.os`, `numpy.sys`) |
| `verifier_agent.py` | `VerifierAgent` and `CheckerAgent` — K-verifier consensus pipeline with `ThreadPoolExecutor` parallelism, domain auto-detection, mechanical aggregation + LLM critique synthesis; all tool guidance from `VerifierConfig` (including scipy/matplotlib) is passed through to verify subagents. `VerifierAgent.verify(problem, solution)` and `CheckerAgent.check(solution)`. Individual verifier exceptions are caught and logged — pipeline continues with remaining results (fails only if all K verifiers crash) |
| `synthesizer.py` | Two-stage consensus synthesis: `aggregate_mechanical()` (majority-vote verdict, mean confidence, issue dedup via `SequenceMatcher`) + `synthesize_critique()` (LLM cleanup with fallback to raw critique concatenation on API failure) |
...
| `domain.py` | `detect_domain(text, *, override=None)` — static keyword dictionary (~1,080 terms, 3 weighted tiers) classifying text as `"math"` or `"physics"` |
| `exceptions.py` | `AlethicError`, `TruncatedResponseError`, `ContextExhaustedError`, `CheckpointError` — custom exception hierarchy for context monitoring |
| `session.py` | Session directory creation (`create_session_dir`), checkpoint write/load (`write_checkpoint`, `load_checkpoint`), incomplete session scanning (`scan_incomplete_sessions`) |
...
| `output.py` | `format_consensus(result, *, mode, command, session_dir)` — formats `ConsensusResult` as box-drawing text, JSON, or single-line quiet output |
| `cli.py` | `argparse`-based CLI (`alethic` entry point) with `solve`/`derive`/`verify`/`check`/`eval` subcommands. Global flags: `--preset`/`-p`, `--file`/`-f`, `--model`/`-m`, `--iterations`/`-n`, `--revisions`, `--confidence-threshold`, `--temperature-{generator,verifier,reviser}`, `--thinking`, `--thinking-budget`, `--max-tokens`, `--best-of`/`-B`, `--tools`, `--no-code`, `--no-balanced`, `--quiet`/`-q`, `--json`, `--api-key`, `--no-stall-reset`, `--stall-window`, `--stall-epsilon`, `--variant-b-model`, `--no-variant-b`, `--resume`, `--context-threshold`, `--search`. Verify/check flags: `--verifiers`/`-K`, `--problem-text`, `--problem-file`/`-P`, `--domain`. `eval run` subcommand: positional `benchmark_file` (path to benchmark JSON), `--preset`, `--search`. Warns to stderr when conflicting `--no-variant-b` and `--variant-b-model` are both specified |
| `eval/__init__.py` | Eval package entry point |
| `eval/harness.py` | `load_benchmark(path) -> dict` — loads and validates benchmark JSON (raises `ValueError` on missing required fields: `id`, `domain`, `problem`, `expected_solvable`). `run_benchmark(path, *, api_key, preset) -> dict` — runs all problems with `MathAgent`/`PhysicsAgent` (domain-dispatch), returns `{name, anchor_sha256, gate_epoch, total, mean_annotation_rate, mean_puct_divergence, results: [...]}` plus every key from `split_metrics`. `split_metrics(results) -> dict` — partitions by `expected_solvable` into two populations: `solve_rate` (**redefined in gate epoch 2** — solved / *solvable* problems only, no longer solved / all; not comparable to a pre-split number), `false_claim_accept_rate` (the primary bias-responsive metric — every "proof" of a false claim is wrong by construction, so an accept is a false positive), `false_claim_reject_rate` (its complement; **not** a false-premise *detection* rate), `false_claim_verdicts`, `n_solvable`, `n_false_claim`, `n_false_claim_scored`, `n_errors`. Errors are asymmetric by design: an errored solvable problem stays in the `solve_rate` denominator (a failure to solve), an errored anchor leaves the anchor denominator (a non-observation — counting it as a rejection would flatter the verifier). Anchor rates are `None`, not `0.0`, when nothing was scored. `anchor_sha256(benchmark) -> str` — SHA-256 over `(id, domain, problem, expected_solvable)` sorted by id; order-invariant, edit-sensitive; says *which problem set ran*. `GATE_EPOCH` — module constant (not a benchmark field) saying *how outcomes were scored*; bump it on any verifier-prompt/model or scoring change, since those never touch the benchmark JSON. Reports are comparable only when both match. `scripts/run_gate.py` imports these rather than reimplementing them. `measure_atoms(events, n_iterations) -> dict` — extracts winning solutions from event logs, parses atom annotations, computes `annotation_rate`, `atom_counts`, `mean_atom_count`. `compute_puct_comparison(events) -> dict` — post-hoc UCB1 scoring on flat candidate pool, measures PUCT vs confidence ranking divergence. Benchmark format: `{name, version, problems: [{id, domain, problem, expected_solvable}]}` |
| `data/benchmarks/math-sample.json` | 10-problem math benchmark covering algebra, number theory, analysis, combinatorics — used for regression gating |
| `data/benchmarks/physics-sample.json` | 10-problem physics benchmark covering classical mechanics, thermodynamics, quantum mechanics, EM/relativity + 1 false claim (Drude model Lorenz number) |
| `examples.py` | Bundled example problems (`python -m alethic.examples`) |
| `__init__.py` | Package entry point — re-exports all public classes (`MathAgent`, `PhysicsAgent`, `VerifierAgent`, `CheckerAgent`, model dataclasses), defines `__all__` and `__version__` |
| `__main__.py` | Enables `python -m alethic` execution, delegates to `cli.main()` |
| `data/domain-keywords.json` | Static keyword dictionary (~1,080 terms, 3 weighted tiers) loaded by `domain.py` via `importlib.resources` |
...
| `skills/alethic-common/references/domain-keywords.json` | Static keyword dictionary (~1,080 terms, 3 weighted tiers) for domain auto-detection |
...
2. **Domain-neutral orchestrator**: The Generate N → Verify all → Select best → Revise loop (including best-of-N sampling) is identical for math and physics. In the Python library, `PhysicsAgent` overrides only the prompt templates via optional kwargs to `generate()`, `verify()`, `revise()` in `subagents.py`. In the skills, both `/alethic-solve` and `/alethic-derive` are thin ~88-line configurators that define domain variables and load a shared `skills/alethic-common/orchestrator.md` (~804 lines). The orchestrator uses placeholders (`{noun}`, `{domain}`, `{verb}`, `{command}`, `{agent_title}`, `{references_dir}`, `{balanced_addendum}`, `{strategy_reset_addendum}`, `{session_skill}`) and reads prompts from the skill's `references/*.md` at runtime. No orchestrator code is duplicated at either level.
...
5. **Structured output parsing**: Verifier output is parsed via regex (`_parse_verification`) for `VERDICT:`, `CONFIDENCE:`, `CRITIQUE:`, `REASON:`, `ISSUES:` fields with independent extraction per field. The `CORRECTED SOLUTION` block regex terminates only on `END CORRECTED SOLUTION` (not arbitrary two-word ALL-CAPS labels like `STEP ONE:`).
6. **Sandboxed code execution with switchable tool guidance**: `execute_python()` runs code in a child subprocess for process-level isolation. Restricted `__builtins__`, an allowlist of importable modules (math, sympy, numpy, scipy, mpmath, matplotlib), and a `_BLOCKED_SUBMODULES` blocklist (`os`, `sys`, `subprocess`, `shutil`, `pathlib`, `socket`, `ctypes`, `signal`) that checks every component of dotted import paths provides defense-in-depth. SymPy is pre-imported as `sp` and NumPy as `np` in the sandbox. Tool-specific guidance (SymPy symbolic verification, NumPy/SciPy numerical verification) is modular and switchable via `--tools` (CLI/skill) or `AgentConfig.tool_guidance` (Python API). In the skills, guidance lives in overlay files (`references/tools/{tool}-{role}.md`) loaded by the orchestrator; in the Python library, guidance strings are stored in `TOOL_GUIDANCE` / `PHYSICS_TOOL_GUIDANCE` maps and appended to system prompts by `_build_system_prompt()`. Generators get advisory toolkits; Verifiers get mandatory re-derivation/spot-check requirements with RED FLAG escalation. Physics overlays additionally reference `sympy.physics.units`, `sympy.physics.quantum`, `scipy.constants`, and `scipy.integrate.solve_ivp`. Default for solve/derive: `sympy,numpy`; default for verify/check: `sympy,numpy,scipy,matplotlib`. Set to `none` to disable. Timeouts enforced at two levels: `signal.SIGALRM` in the child process and `subprocess.run(timeout=)` in the parent. Thread-safe.
...
19. **Executable intermediate steps**: The Generator is prompted to write `verify_step_N()` Python functions for key intermediate results (e.g., after a Fourier transform, after an integration-by-parts step) and embed the numerical outputs inline in the solution text. The Verifier is instructed to treat these embedded check results as ground truth — if `verify_step_3()` prints a confirmed value, the verifier skips re-deriving that step and focuses verification effort on logic, citations, and the final result. This reduces wasted verification compute on algebraically-verified sub-steps. Implemented as prompt additions to `GENERATOR_SYSTEM` / `PHYSICS_GENERATOR_SYSTEM` and corresponding `VERIFIER_SYSTEM` prompts in `prompts.py`/`physics_prompts.py`, and in the skill reference files `generator.md`/`verifier.md`.

20. **Error taxonomy-driven revision**: After each failed verification, `classify_errors(critique)` in `error_taxonomy.py` applies a keyword heuristic to the verifier critique to identify the dominant error category (algebra/logic/citation/interpretation/units/missing_case). A category-specific revision strategy addendum is injected into the reviser's user prompt via the `critique_addendum` parameter added to `revise()` in `subagents.py`. For example, an `algebra` classification injects "Focus revision on: recomputing all algebraic manipulations from scratch..."; a `citation` classification injects "Focus revision on: providing explicit citations or proofs for every invoked theorem...". This steers the reviser's attention rather than leaving it to re-read the full critique. No extra LLM call — classification is deterministic.

21. **Backward verification pass**: All verifier prompts include a mandatory backward pass after the standard forward verification: starting from the conclusion and tracing each inference backward toward the premises. This catches circular reasoning (conclusion assumed in a premise) and unjustified logical leaps that are easy to miss in forward reading (since the forward pass confirms each step conditional on the previous one being correct). Implemented as an added section in `VERIFIER_SYSTEM` / `PHYSICS_VERIFIER_SYSTEM` in `prompts.py`/`physics_prompts.py` and in the skill verifier reference files.

22. **Adversarial verifier self-correction**: When `AgentConfig.adversarial_self_correction` is True (default in `thorough`/`extreme` presets), the verifier's system prompt includes a 5-round internal self-critique protocol: (1) initial assessment, (2) hallucination/false-positive check ("am I being too harsh?"), (3) revised assessment, (4) completeness check ("have I missed anything?"), (5) final verdict. This addresses verifier overconfidence and false negatives — the verifier must argue with itself before committing to a verdict. Injected via `extra_system` parameter on `verify()` in `subagents.py`. `MathAgent._adversarial_addendum()` returns `ADVERSARIAL_VERIFIER_ADDENDUM`; `PhysicsAgent._adversarial_addendum()` returns `PHYSICS_ADVERSARIAL_VERIFIER_ADDENDUM`. Motivated by Woodruff et al. ([arXiv:2602.03837](https://arxiv.org/abs/2602.03837)) case studies on adversarial verifier correction.

23. **Eval harness with benchmark gating**: `alethic eval run --benchmark path/to/bench.json` runs a benchmark of math/physics problems (JSON format: `{name, version, problems: [{id, domain, problem, expected_solvable}]}`), collecting solve rates, per-problem verdicts, and confidence scores. `load_benchmark()` validates required fields and raises `ValueError` on malformed entries. `run_benchmark()` dispatches `MathAgent` or `PhysicsAgent` based on each problem's `domain` field. Output: `{name, total, solve_rate, results: [...]}`. Benchmark data lives in `data/benchmarks/`. The bundled `math-sample.json` (10 problems) serves as a regression baseline for CI gating. Python API: `from alethic.eval.harness import run_benchmark`.
