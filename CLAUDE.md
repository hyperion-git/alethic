...
```bash
DEST=~/.claude/plugins/cache/local/alethic/3.0.5
mkdir -p "$DEST"
...
| Preset | Iters | Revs | Threshold | Best-of | Thinking | Think budget | Max tokens | Stall reset | Window | Epsilon | N boost | Variant B | Ctx thresh |
|--------|-------|------|-----------|---------|----------|-------------|------------|-------------|--------|---------|---------|-----------|------------|
| `quick` | 2 | 1 | 0.85 | 1 | off | — | 16,384 | off | — | — | 0 | — | 0.85 |
| `default` | 5 | 3 | 0.90 | 2 | off | — | 16,384 | on | 2 | 0.03 | 1 | — | 0.80 |
| `thorough` | 8 | 5 | 0.95 | 3 | on | 15,000 | 32,768 | on | 3 | 0.02 | 1 | Sonnet | 0.80 |
| `extreme` | 12 | 5 | 0.97 | 5 | on | 40,000 | 65,536 | on | 3 | 0.02 | 2 | Sonnet | 0.75 |
...
| `cli.py` | `argparse`-based CLI (`alethic` entry point) with `solve`/`derive`/`verify`/`check` subcommands. Global flags: `--preset`/`-p`, `--file`/`-f`, `--model`/`-m`, `--iterations`/`-n`, `--revisions`, `--confidence-threshold`, `--temperature-{generator,verifier,reviser}`, `--thinking`, `--thinking-budget`, `--max-tokens`, `--best-of`/`-B`, `--tools`, `--no-code`, `--no-balanced`, `--quiet`/`-q`, `--json`, `--api-key`, `--no-stall-reset`, `--stall-window`, `--stall-epsilon`, `--variant-b-model`, `--no-variant-b`, `--resume`, `--context-threshold`. Verify/check flags: `--verifiers`/`-K`, `--problem-text`, `--problem-file`/`-P`, `--domain`. Warns to stderr when conflicting `--no-variant-b` and `--variant-b-model` are both specified |
