...
def linear_drift_phase():
    ...
    y(t) = D => x(t) = D * t  =>  AVAR should give zero for m=1, but
    Hadamard should give zero for all m (insensitive to linear drift).
    """
...
class TestAvar:
    ...
    def test_freq_input(self, white_phase_noise):
        """Frequency-domain input should produce a valid result."""
    ...
    def test_m_too_large(self):
        x = np.zeros(10)
        with pytest.raises(ValueError, match="at least"):
            avar(x, m=5, tau_0=1.0)  # needs 2*5+1 = 11 samples
...
class TestHvar:
    ...
    def test_insensitive_to_linear_drift(self, linear_drift_phase):
        """Hadamard variance of a pure linear drift should be zero."""
...
# ---------------------------------------------------------------------------
# Tau scaling (white noise property checks)
# ---------------------------------------------------------------------------
...
class TestTauScaling:
    ...
    def test_avar_scales_as_tau_minus2(self, white_phase_noise):
        ...
        # ratio of variances should be ~4 (tau^-2 scaling)
    ...
    def test_mvar_scales_as_tau_minus3(self, white_phase_noise):
        ...
        # ratio should be ~8 (tau^-3 scaling, factor of 2 in tau)
...
class TestValidation:
    ...
    def test_list_input(self):
        """Plain Python lists should work."""
