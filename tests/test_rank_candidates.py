...
def test_rank_candidates_called_during_verify(monkeypatch):
    """rank_candidates() should be called within _verify_candidates via router."""
    ...
    call_log = []
    _original = ma.router.rank_candidates

    def tracking_rank(verifications):
        call_log.append(len(verifications))
        return _original(verifications)

    monkeypatch.setattr(ma.router, "rank_candidates", tracking_rank)
