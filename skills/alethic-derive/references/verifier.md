...
(Omit this section if the derivation has no ATOM markers)

SECTION CONFIDENCES:
- [section name]: [0.0-1.0] [optional note]
(Omit this section if the solution is too short to decompose into sections)

If the derivation contains ATOM[N] markers, report your confidence in each atom in the ATOM CONFIDENCES block using format `ATOM[N]: 0.NN optional note`. Omit this section if the derivation has no atom markers.
...
- `ALETHIC_L1_CHECK: FAILURE` → `[MAJOR]` — a correct formula must pass all limits
- `ALETHIC_L2_CHECK: FAILURE` → `[MAJOR]` — symbolic and numeric values disagree
